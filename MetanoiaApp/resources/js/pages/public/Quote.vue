...
const serviceIcons: Record<string, string> = { supply: 'M3 7l9-4 9 4-9 4-9-4Z', install: 'M14.7 6.3a4 4 0 0 0-5.4 5.4l-6 6 2 2 6-6a4 4 0 0 0 5.4-5.4l-2.3 2.3-2-2 2.3-2.3Z', both: 'M20 6 9 17l-5-5' };

const whatsappHref = computed(() => {
    const f = form;
    const lines = [
        'Hello Metanoia Energy, I would like a quotation.',
        `Name: ${f.salutation} ${f.name}`.trim(),
        f.phone ? `Phone: ${f.phone}` : '',
        f.company ? `Company: ${f.company}` : '',
        f.segment ? `Segment: ${props.options.segments[f.segment] ?? f.segment}` : '',
        f.location ? `Location: ${f.location}` : '',
        f.service ? `Service: ${props.options.services[f.service] ?? f.service}` : '',
        f.system_type ? `System: ${props.options.systemTypes[f.system_type] ?? ''}` : '',
...
const steps = [
    { n: '1', t: 'You submit', icon: 'M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z' },
    { n: '2', t: 'We review', icon: 'm9 11 3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11' },
    { n: '3', t: 'You get a quote', icon: 'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Zm0 0v6h6' },
];
</script>

<template>
    <Head title="Request a Quote — Metanoia Energy">
        <meta name="description" content="Request a free, tailored solar quotation from Metanoia Energy. Answer a few quick questions about your factory, farm, or business." />
    </Head>
    ...
    <PublicLayout>
        ...
        <section class="bg-brand-beige/50">
            <div class="container-x max-w-3xl py-14 lg:py-16">
                ...
                <h1 class="mt-5 font-display text-4xl font-extrabold leading-[1.1] tracking-tight text-brand-blue">Tell us about your project</h1>
                <p class="lead mt-4">Answer a few quick questions and our team replies with a tailored quotation. Free, no obligation, about 2 minutes.</p>
            </div>
        </section>
        ...
        <section class="section">
            <div class="container-x max-w-3xl">
                ...
                <div class="mb-8 grid grid-cols-3 gap-3">
                    <div v-for="s in steps" :key="s.n" class="rounded-card border border-brand-border bg-white p-4">
                        ...
                        <p class="mt-2 font-display text-sm font-semibold text-brand-navy">{{ s.t }}</p>
                    </div>
                </div>

                <form class="rounded-card border border-brand-border bg-white p-6 md:p-8" @submit.prevent="submit">
                    <!-- About you -->
                    <h2 class="q-section">About you</h2>
                    <div class="grid gap-x-5 sm:grid-cols-2">
                        <div class="q-field">
                            <label class="q-lbl">Title</label>
                            <select v-model="form.salutation" class="q-ctrl">
                                <option value="">—</option>
                                <option v-for="s in options.salutations" :key="s" :value="s">{{ s }}</option>
                            </select>
                        </div>
                        <div class="q-field">
                            <label class="q-lbl">Full name <span class="q-req">*</span></label>
                            <input v-model="form.name" type="text" class="q-ctrl" placeholder="e.g. Tarek Said" />
                            <p v-if="form.errors.name" class="q-err">{{ form.errors.name }}</p>
                        </div>
                        <div class="q-field">
                            <label class="q-lbl">Phone / WhatsApp <span class="q-req">*</span></label>
                            <input v-model="form.phone" type="tel" class="q-ctrl" placeholder="01x xxxx xxxx" />
                            <p v-if="form.errors.phone" class="q-err">{{ form.errors.phone }}</p>
                        </div>
                        <div class="q-field">
                            <label class="q-lbl">Email</label>
                            <input v-model="form.email" type="email" class="q-ctrl" placeholder="name@company.com" />
                            <p v-if="form.errors.email" class="q-err">{{ form.errors.email }}</p>
                        </div>
                        <div class="q-field sm:col-span-2">
                            <label class="q-lbl">Company / business name</label>
                            <input v-model="form.company" type="text" class="q-ctrl" placeholder="Optional" />
                        </div>
                    </div>

                    <!-- Business -->
                    <h2 class="q-section">Your business</h2>
                    <div class="grid gap-x-5 sm:grid-cols-2">
                        <div class="q-field">
                            <label class="q-lbl">I am a… <span class="q-req">*</span></label>
                        ...
                        <div class="q-field">
                            <label class="q-lbl">Site location (governorate / city) <span class="q-req">*</span></label>
                            <input v-model="form.location" type="text" class="q-ctrl" placeholder="e.g. Sadat City" />
                            <p v-if="form.errors.location" class="q-err">{{ form.errors.location }}</p>
                        </div>
                    </div>

                    <!-- Needs -->
                    <h2 class="q-section">What you need</h2>
                    <div class="q-field">
                        <label class="q-lbl">Service needed <span class="q-req">*</span></label>
                        <div class="grid grid-cols-3 gap-2">
                            <button v-for="(label, key) in options.services" :key="key" type="button" class="q-seg" :class="{ 'q-seg--on': form.service === key }" @click="form.service = key">
                                ...
                                {{ key === 'both' ? 'Both' : key === 'supply' ? 'Supply' : 'Install' }}
                            </button>
                        </div>
                    ...
                    <div class="grid gap-x-5 sm:grid-cols-2">
                        <div class="q-field">
                            <label class="q-lbl">System type</label>
                        ...
                        <div class="q-field">
                            <label class="q-lbl">Avg. monthly electricity bill</label>
                        ...
                        <div class="q-field">
                            <label class="q-lbl">When to start</label>
                        ...
                        <div class="q-field">
                            <label class="q-lbl">Preferred contact</label>
                        ...
                        <div class="q-field sm:col-span-2">
                            <label class="q-lbl">Project details</label>
                            <textarea v-model="form.details" rows="3" class="q-ctrl" placeholder="Anything else we should know? (optional)"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
                        <button type="submit" class="btn btn--primary" :disabled="form.processing">
                            ...
                            {{ form.processing ? 'Sending…' : 'Request my quote' }}
                        </button>
                    ...
                    <p class="mt-3 text-xs text-brand-muted">We use your details only to prepare and send your quotation.</p>
                </form>
            </div>
        </section>
    </PublicLayout>
